...
const htmlReportDir = path.resolve(process.cwd(), "reports/cucumber-htmlreport.html");
...
function generateReport() {
  if (!fs.existsSync(cucumberJsonDir)) {
    ...
    report.generate({
      ...
      useCDN: true,
      pageTitle: "Pokerstars report",
      reportName: `Pokerstars report - ${new Date().toLocaleString()}`,
      metadata: {
        app: {
          name: "Pokerstars report",
          version: "1",
        },
        browser: {
          name: "chrome",
      ...
      customData: {
        title: "Run info",
        data: [
          { label: "Project", value: "Pokerstars" },
